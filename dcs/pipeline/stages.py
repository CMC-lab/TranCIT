...
from dcs.utils import compute_event_statistics, extract_event_snapshots, remove_artifact_trials
from dcs.utils.signal import find_peak_locations, shrink_locations_resample_uniform, find_best_shrinked_locations
...
class InputValidationStage(PipelineStage):
    ...
    def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        if original_signal.shape[1] != detection_signal.shape[1]:
            logger.warning("original_signal and detection_signal must have the same time dimension")
        
        self._log_stage_complete("input validation")
        return {"original_signal": original_signal, "detection_signal": detection_signal}
        ...
        if self.config.options.detection:
            ...
            if align_type == "peak":
                ...
                if self.config.detection.shrink_flag:
                    ...
                    temp_locs_shrink = shrink_locations_resample_uniform(temp_locs, pool_window)
                    locs, _ = find_best_shrinked_locations(D_for_detection, temp_locs_shrink, temp_locs)
...
class SnapshotExtractionStage(PipelineStage):
    ...
    def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Extracted event snapshots with shape: {event_snapshots.shape}")
    ...
    def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        if self.config.detection.remove_artif:
            try:
                ...
                if removed_count > 0:
                    logger.info(f"Removed {removed_count} artifact trials. {cleaned_snapshots.shape[2]} trials remaining.")
        ...
        try:
            ...
            for n_btsp in range(1, self.config.monte_carlo.n_btsp + 1):
                try:
                    ...
                    btsp_stats = compute_event_statistics(btsp_snapshots, kwargs["morder"])
...
class DeSnapAnalysisStage(PipelineStage):
    ...
    def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        return {"desnap_full_output": desnap_full_output, "event_stats_unconditional": event_stats_unconditional}
...
class OutputPreparationStage(PipelineStage):
    ...
    def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        if kwargs.get("bootstrap_causal_outputs_list") is not None:
            final_results["BootstrapCausalOutputs"] = kwargs["bootstrap_causal_outputs_list"]
        ...
        if kwargs.get("event_stats_unconditional") is not None:
            final_results["Yt_stats_unconditional"] = kwargs["event_stats_unconditional"]
