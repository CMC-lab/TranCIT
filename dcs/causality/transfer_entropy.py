    ...
    def analyze(self, data: np.ndarray, **kwargs) -> TransferEntropyResult:
        ...
        **kwargs
        ...
        try:
            ...
            raise ComputationError(f"Transfer Entropy analysis failed: {e}", "te_computation", data.shape)
    ...
    def _compute_inhomogeneous_te(
        ...
        for t in range(n_time_steps):
            try:
                ...
                result_arrays["transfer_entropy"][t, 1] = 0.5 * np.log(result_arrays["te_residual_cov"][t, 1] / sigy)
                result_arrays["transfer_entropy"][t, 0] = 0.5 * np.log(result_arrays["te_residual_cov"][t, 0] / sigx)
                
            except Exception as e:
                logger.error(f"Transfer Entropy computation failed at time step {t}: {e}")
