...
class RelativeDCSCalculator(BaseAnalyzer):
    ...
    def analyze(self, event_data: np.ndarray, stats: Dict, **kwargs) -> RelativeDCSResult:
        """
        ...
        **kwargs
        ...
        try:
            ...
            causality_measures = time_varying_causality(event_data, stats, causal_params)
            ...
            raise ComputationError(f"Relative DCS analysis failed: {e}", "rdcs_computation", event_data.shape)
    
    def _validate_config(self) -> None:
        """Validate configuration parameters."""
        if self.config["model_order"] <= 0:
            raise ValidationError("model_order must be positive", "model_order", self.config["model_order"])
        
        if self.config["reference_time"] < 0:
            raise ValidationError("reference_time must be non-negative", "reference_time", self.config["reference_time"])
        
        if self.config["estimation_mode"] not in ["OLS", "RLS"]:
            raise ValidationError("estimation_mode must be 'OLS' or 'RLS'", "estimation_mode", self.config["estimation_mode"])
    ...
    def _validate_input_data(self, event_data: np.ndarray, stats: Dict) -> None:
        ...
        if event_data.ndim != 3:
            raise ValidationError("event_data must be 3D", "event_data_ndim", event_data.ndim)
        ...
        for key in required_keys:
            if key not in stats:
                raise ValidationError(f"stats must contain '{key}'", "stats_missing_key", key)
...
def time_varying_causality(
    ...
    logger.info(f"Computing time-varying causality: mode={estim_mode}, morder={morder}, ref_time={ref_time}")
    ...
    for t in range(nobs):
        try:
            _compute_causality_at_timepoint(
                t, event_data, stats, causality_measures,
                nvar, ntrials, ref_time, estim_mode, morder, diag_flag, old_version
            )
        except Exception as e:
...
def _validate_time_varying_causality_inputs(
    ...
    if not isinstance(event_data, np.ndarray):
        raise ValidationError("event_data must be a NumPy array", "event_data_type", type(event_data))
    ...
    if not isinstance(causal_params, dict):
        raise ValidationError("causal_params must be a dictionary", "causal_params_type", type(causal_params))
    
    required_params = ["ref_time", "estim_mode", "morder", "diag_flag", "old_version"]
    for param in required_params:
        if param not in causal_params:
            raise ValidationError(f"causal_params must contain '{param}'", "causal_params_missing", param)
    ...
    if causal_params["ref_time"] < 0:
        raise ValidationError("ref_time must be non-negative", "ref_time", causal_params["ref_time"])
...
def _compute_causality_at_timepoint(
    ...
    cov_xp = stats["Sigma"][t, 3::2, 3::2]  # X past covariance
    cov_yp = stats["Sigma"][t, 2::2, 2::2]  # Y past covariance
    c_xyp = stats["Sigma"][t, 3::2, 2::2]   # X past - Y past cross-covariance
    c_yxp = stats["Sigma"][t, 2::2, 3::2]   # Y past - X past cross-covariance
    mean_xp = stats["mean"][3::2, t]         # X past mean
    mean_yp = stats["mean"][2::2, t]         # Y past mean

    cov_xp_reg = regularize_if_singular(cov_xp)
    cov_yp_reg = regularize_if_singular(cov_yp)

    _compute_transfer_entropy(
        t, b, c, sigy, sigx, cov_xp, cov_yp, c_xyp, c_yxp, 
        cov_xp_reg, cov_yp_reg, causality_measures
    )
    ...
    _compute_causal_strength_measures(
        t, b, c, sigy, sigx, cov_xp, cov_yp, cov_xp_ref, cov_yp_ref,
        ref_cov_xp, ref_cov_yp, lagged_vars, stats, ntrials, ref_time,
        diag_flag, old_version, causality_measures
    )
    ...
    if not diag_flag:
        ...
        if old_version:
            ...
            _compute_new_version_rdcs(
                t, b, c, sigy, sigx, cov_xp_ref, cov_yp_ref, ref_cov_xp, ref_cov_yp,
                causality_measures
            )
    else:
        causality_measures["DCS"][t, 1] = 0.5 * np.log(
...
def _compute_old_version_rdcs(
    ...
    causality_measures["rDCS"][t, 1] = (
        ...
        - 0.5
        + 0.5 * (sigy + b.T @ cov_xp_lag[1::2, 1::2] @ b) / (sigy + b.T @ ref_cov_xp @ b)
    )

    causality_measures["rDCS"][t, 0] = (
        (0.5 * np.log((sigx + c.T @ ref_cov_yp @ c) / sigx))
        - 0.5
        + (0.5 * (sigx + c.T @ cov_xp_lag[0::2, 0::2] @ c) / (sigx + c.T @ ref_cov_yp @ c))
    )
...
def _compute_old_version_rdcs_diagonal(
    ...
    causality_measures["rDCS"][t, 1] = (
        ...
        - 0.5
        + (0.5 * (sigy + b.T @ np.diag(np.diag(cov_xp_lag[1::2, 1::2])) @ b) / (sigy + b.T @ np.diag(np.diag(ref_cov_xp)) @ b))
    )
    
    causality_measures["rDCS"][t, 0] = (
        (0.5 * np.log((sigx + c.T @ np.diag(np.diag(ref_cov_yp)) @ c) / sigx))
        - 0.5
        + (0.5 * (sigx + c.T @ np.diag(np.diag(cov_xp_lag[0::2, 0::2])) @ c) / (sigx + c.T @ np.diag(np.diag(ref_cov_yp)) @ c))
    )
...
def _compute_new_version_rdcs_diagonal(
    ...
    causality_measures["rDCS"][t, 1] = (
        (0.5 * np.log((sigy + b.T @ np.diag(np.diag(ref_cov_xp)) @ b) / sigy))
        - 0.5
        + (0.5 * (sigy + b.T @ np.diag(np.diag(cov_xp_ref)) @ b) / (sigy + b.T @ np.diag(np.diag(ref_cov_xp)) @ b))
    )
    
    causality_measures["rDCS"][t, 0] = (
        (0.5 * np.log((sigx + c.T @ np.diag(np.diag(ref_cov_yp)) @ c) / sigx))
        - 0.5
        + (0.5 * (sigx + c.T @ np.diag(np.diag(cov_yp_ref)) @ c) / (sigx + c.T @ np.diag(np.diag(ref_cov_yp)) @ c))
    ) 
