...
def time_varying_causality(Yt_event, Yt_stats, CausalParams):
    ...
    CausalOutput = {'TE': np.zeros((nobs, 2)), 'DCS': np.zeros((nobs, 2)), 'rDCS': np.zeros((nobs, 2))}
    ...
    for t in range(nobs):
        ...
        CausalOutput['TE'][t, 1] = 0.5 * np.log((sigy + b.T @ cov_Xp @ b - b.T @ C_XYp @ np.linalg.inv(cov_Yp) @ C_XYp.T @ b) / sigy)
        CausalOutput['TE'][t, 0] = 0.5 * np.log((sigx + c.T @ cov_Yp @ c - c.T @ C_YXp @ np.linalg.inv(cov_Xp) @ C_YXp.T @ c) / sigx)

        mean_X_ref = np.mean(Yt_stats['mean'][3::2, ref_time], axis=1)
        mean_Y_ref = np.mean(Yt_stats['mean'][2::2, ref_time], axis=1)

        cov_Xp_ref = cov_Xp + np.outer(mean_Xp, mean_Xp) - np.outer(mean_Xp, mean_X_ref) - np.outer(mean_X_ref, mean_Xp) + np.outer(mean_X_ref, mean_X_ref)
        cov_Yp_ref = cov_Yp + np.outer(mean_Yp, mean_Yp) - np.outer(mean_Yp, mean_Y_ref) - np.outer(mean_Y_ref, mean_Yp) + np.outer(mean_Y_ref, mean_Y_ref)
        ...
        if sigy == 0:
            sigy = np.finfo(float).eps  # Smallest positive float to avoid divide by zero
        if sigx == 0:
        ...
        if not CausalParams['diag_flag']:
            ...
            if CausalParams['old_version']:
                CausalOutput['rDCS'][t, 1] = 0.5 * np.log((sigy + b.T @ np.mean(Yt_stats['Sigma'][ref_time, 3::2, 3::2], axis=0) @ b) / sigy) - 0.5 + \
                                              0.5 * (sigy + b.T @ cov_Xp_lag[1::2, 1::2] @ b) / (sigy + b.T @ np.mean(Yt_stats['Sigma'][ref_time, 3::2, 3::2], axis=0) @ b)

                CausalOutput['rDCS'][t, 0] = 0.5 * np.log((sigx + c.T @ np.mean(Yt_stats['Sigma'][ref_time, 2::2, 2::2], axis=0) @ c) / sigx) - 0.5 + \
                                              0.5 * (sigx + c.T @ cov_Xp_lag[0::2, 0::2] @ c) / (sigx + c.T @ np.mean(Yt_stats['Sigma'][ref_time, 2::2, 2::2], axis=0) @ c)
            else:
                CausalOutput['rDCS'][t, 1] = 0.5 * np.log((sigy + b.T @ np.mean(Yt_stats['Sigma'][ref_time, 3::2, 3::2], axis=0) @ b) / sigy) - 0.5 + \
                                              0.5 * (sigy + b.T @ cov_Xp_ref @ b) / (sigy + b.T @ np.mean(Yt_stats['Sigma'][ref_time, 3::2, 3::2], axis=0) @ b)

                CausalOutput['rDCS'][t, 0] = 0.5 * np.log((sigx + c.T @ np.mean(Yt_stats['Sigma'][ref_time, 2::2, 2::2], axis=0) @ c) / sigx) - 0.5 + \
                                              0.5 * (sigx + c.T @ cov_Yp_ref @ c) / (sigx + c.T @ np.mean(Yt_stats['Sigma'][ref_time, 2::2, 2::2], axis=0) @ c)
        else:
            ...
            if CausalParams['old_version']:
                CausalOutput['rDCS'][t, 1] = 0.5 * np.log((sigy + b.T @ np.diag(np.diag(np.mean(Yt_stats['Sigma'][ref_time, 3::2, 3::2], axis=0))) @ b) / sigy) - 0.5 + \
                                              0.5 * (sigy + b.T @ np.diag(np.diag(cov_Xp_lag[1::2, 1::2])) @ b) / (sigy + b.T @ np.diag(np.diag(np.mean(Yt_stats['Sigma'][ref_time, 3::2, 3::2], axis=0))) @ b)

                CausalOutput['rDCS'][t, 0] = 0.5 * np.log((sigx + c.T @ np.diag(np.diag(np.mean(Yt_stats['Sigma'][ref_time, 2::2, 2::2], axis=0))) @ c) / sigx) - 0.5 + \
                                              0.5 * (sigx + c.T @ np.diag(np.diag(cov_Xp_lag[0::2, 0::2])) @ c) / (sigx + c.T @ np.diag(np.diag(np.mean(Yt_stats['Sigma'][ref_time, 2::2, 2::2], axis=0))) @ c)
            else:
                CausalOutput['rDCS'][t, 1] = 0.5 * np.log((sigy + b.T @ np.diag(np.diag(np.mean(Yt_stats['Sigma'][ref_time, 3::2, 3::2], axis=0))) @ b) / sigy) - 0.5 + \
                                              0.5 * (sigy + b.T @ np.diag(np.diag(cov_Xp_ref)) @ b) / (sigy + b.T @ np.diag(np.diag(np.mean(Yt_stats['Sigma'][ref_time, 3::2, 3::2], axis=0))) @ b)

                CausalOutput['rDCS'][t, 0] = 0.5 * np.log((sigx + c.T @ np.diag(np.diag(np.mean(Yt_stats['Sigma'][ref_time, 2::2, 2::2], axis=0))) @ c) / sigx) - 0.5 + \
                                              0.5 * (sigx + c.T @ np.diag(np.diag(cov_Yp_ref)) @ c) / (sigx + c.T @ np.diag(np.diag(np.mean(Yt_stats['Sigma'][ref_time, 2::2, 2::2], axis=0))) @ c)
