...
from dcs.utils.helpers import estimate_coefficients
...
class GrangerCausalityCalculator(BaseAnalyzer):
    ...
    def analyze(self, data: np.ndarray, **kwargs) -> GrangerCausalityResult:
        ...
        **kwargs
        ...
        try:
            current_data, lagged_data, n_time_steps = self._prepare_data_structures(data)
            ...
            if self.config["time_mode"] == "inhomo":
                self._compute_inhomogeneous_gc(
                    current_data, lagged_data, result_arrays, data.shape[2], data.shape[0]
                )
            ...
            raise ComputationError(f"Granger Causality analysis failed: {e}", "gc_computation", data.shape)
    ...
    def _validate_config(self) -> None:
        ...
        if self.config["time_mode"] not in ["inhomo", "homo"]:
            raise ValidationError("time_mode must be 'inhomo' or 'homo'", "time_mode", self.config["time_mode"])
    ...
    def _validate_input_data(self, data: np.ndarray) -> None:
        ...
        if data.ndim != 3:
            raise ValidationError("Input data must be 3D (n_vars, n_observations, n_trials)", "data_ndim", data.ndim)
        
        if data.shape[0] != 2:
            raise ValidationError("Input data must be bivariate (n_vars=2)", "n_vars", data.shape[0])
    ...
    def _prepare_data_structures(
        ...
        lagged_data = extended_data[:, 1 : self.config["model_order"] + 1, r1 - 1 : nobs - 1, :]
    ...
    def _initialize_result_arrays(
        ...
        return {
            ...
            "coefficients": np.full((nobs, nvar, nvar * self.config["model_order"] + 1), np.nan),
        ...
        for t in range(n_time_steps):
            try:    
                coeff, residual_cov = estimate_coefficients(
                    current_data[:, t, :].T,
                    lagged_data[:, :self.config["model_order"], t, :].reshape(nvar * self.config["model_order"], ntrials).T,
                    ntrials,
                )
                ...
                result_arrays["granger_causality"][t, 1] = np.log(sigy_reduced / sigy)  # X -> Y
                result_arrays["granger_causality"][t, 0] = np.log(sigx_reduced / sigx)  # Y -> X
                
            except Exception as e:
                logger.error(f"Granger Causality computation failed at time step {t}: {e}")
    ...
    def _compute_homogeneous_gc(
        ...
        lagged_x_only = lagged_data[1, :, :, :].reshape(self.config["model_order"], -1).T
        lagged_y_only = lagged_data[0, :, :, :].reshape(self.config["model_order"], -1).T
