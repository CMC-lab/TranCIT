...
class ModelValidator(BaseAnalyzer):
    ...
    def validate(
        ...
        **kwargs
        ...
        try:
            ...
            raise ComputationError(f"Model validation failed: {e}", "validation_computation", coefficients.shape)
    
    def _validate_config(self) -> None:
        """Validate configuration parameters."""
        if self.config["max_lag"] <= 0:
            raise ValidationError("max_lag must be positive", "max_lag", self.config["max_lag"])
        
        if not 0 < self.config["significance_level"] < 1:
            raise ValidationError("significance_level must be between 0 and 1", "significance_level", self.config["significance_level"])
        
        if self.config["stability_threshold"] <= 0:
            raise ValidationError("stability_threshold must be positive", "stability_threshold", self.config["stability_threshold"])
    ...
    def _validate_input_data(
        ...
        if not isinstance(residuals, np.ndarray):
            raise ValidationError("residuals must be a NumPy array", "residuals_type", type(residuals))
        
        if not isinstance(design_matrix, np.ndarray):
            raise ValidationError("design_matrix must be a NumPy array", "design_matrix_type", type(design_matrix))
        ...
        for var in range(residuals.shape[1]):
            ...
            if len(series) > 3:
                ...
                try:
                    ...
                    normality_tests[f"anderson_var_{var}"] = result.significance_level[2]  # 5% level
                except:
    ...
    def _check_model_stability(self, coefficients: np.ndarray) -> bool:
        ...
        try:
            ...
            for i in range(model_order - 1):
                companion_matrix[i * n_vars:(i + 1) * n_vars, (i + 1) * n_vars:(i + 2) * n_vars] = np.eye(n_vars)
            
            for i in range(model_order):
                start_idx = i * n_vars
                end_idx = (i + 1) * n_vars
                companion_matrix[(model_order - 1) * n_vars:, start_idx:end_idx] = coefficients[:, i * n_vars:(i + 1) * n_vars]
    ...
    def _compute_log_likelihood(self, residuals: np.ndarray) -> float:
        ...
        try:
            ...
            log_likelihood = -0.5 * n_obs * (n_vars * np.log(2 * np.pi) + log_det + np.mean(mahal_dist))
    ...
    def _compute_aic(self, log_likelihood: float, n_parameters: int, n_observations: int) -> float:
        """
    ...
    def _compute_bic(self, log_likelihood: float, n_parameters: int, n_observations: int) -> float:
        """
