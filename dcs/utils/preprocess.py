...
def remove_artifact_trials(
    ...
    if not isinstance(lower_threshold, (int, float)):
        raise ValidationError("lower_threshold must be a number", "lower_threshold_type", type(lower_threshold))
    
    # Check dimensions
    if event_data.shape[2] != len(locations):
        raise ValidationError("event_data trials dimension must match locations length", "dimension_mismatch", (event_data.shape[2], len(locations)))
    
    if event_data.shape[0] < 2:
        raise ValidationError("event_data must have at least 2 variables", "n_vars", event_data.shape[0])
    ...
    try:
        ...
        logger.info(f"Removed {len(trials_to_remove)} artifact trials (threshold: {lower_threshold})")
        ...
        raise ComputationError(f"Artifact removal failed: {e}", "artifact_removal", None)
...
def regularize_if_singular(
    ...
    if not isinstance(matrix, np.ndarray) or matrix.ndim != 2:
        raise ValidationError("matrix must be a 2D numpy array", "matrix_ndim", matrix.ndim if hasattr(matrix, 'ndim') else None)
    ...
    if not isinstance(threshold, (int, float)) or threshold <= 0:
        raise ValidationError("threshold must be a positive number", "threshold", threshold)
    ...
    try:
        ...
        if abs(det) < threshold:
            logger.warning(f"Singular matrix detected (det={det:.2e}), applying regularization")
            ...
            if samples is not None and samples > matrix.shape[0]:
                try:
                    ...
                    logger.warning(f"Ledoit-Wolf regularization failed: {e}, using diagonal regularization")
        ...
        raise ComputationError(f"Matrix regularization failed: {e}", "matrix_regularization", None)
...
def validate_data_quality(
    ...
    if not isinstance(check_nan, bool):
        raise ValidationError("check_nan must be a boolean", "check_nan_type", type(check_nan))
    
    if not isinstance(check_inf, bool):
        raise ValidationError("check_inf must be a boolean", "check_inf_type", type(check_inf))
    
    if not isinstance(check_constant, bool):
        raise ValidationError("check_constant must be a boolean", "check_constant_type", type(check_constant))
...
def normalize_data(
    ...
    if method not in ["zscore", "minmax", "robust", "none"]:
        raise ValidationError("method must be one of: 'zscore', 'minmax', 'robust', 'none'", "method", method)
    
    if axis is not None and not isinstance(axis, int):
        raise ValidationError("axis must be an integer or None", "axis_type", type(axis))
    ...
    try:
        ...
        if method == "zscore":
            ...
            mad_val = np.median(np.abs(data_flat - median_val), axis=axis, keepdims=True)
        ...
        raise ComputationError(f"Data normalization failed: {e}", "data_normalization", None)
