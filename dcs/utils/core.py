...
def extract_event_windows(
    ...
    if not isinstance(signal, np.ndarray) or signal.ndim != 1:
        raise ValidationError("signal must be a 1D numpy array", "signal_ndim", signal.ndim if hasattr(signal, 'ndim') else None)
    
    if not isinstance(centers, np.ndarray) or centers.ndim != 1:
        raise ValidationError("centers must be a 1D numpy array", "centers_ndim", centers.ndim if hasattr(centers, 'ndim') else None)
    
    if not isinstance(start_offset, int):
        raise ValidationError("start_offset must be an integer", "start_offset_type", type(start_offset))
    
    if not isinstance(window_length, int) or window_length <= 0:
        raise ValidationError("window_length must be a positive integer", "window_length", window_length)
...
def compute_event_statistics(
    ...
    if not isinstance(event_data, np.ndarray) or event_data.ndim != 3:
        raise ValidationError("event_data must be a 3D numpy array", "event_data_ndim", event_data.ndim if hasattr(event_data, 'ndim') else None)
    ...
    if event_data.shape[0] != nvar * (model_order + 1):
        raise ValidationError(
            ...
            "event_data_shape", event_data.shape
        )
    
    if event_data.shape[2] < 2:
        raise ValidationError("At least 2 trials required for statistics computation", "n_trials", event_data.shape[2])
    ...
    try:
        ...
        raise ComputationError(f"Statistics computation failed: {e}", "statistics_computation", None)
...
def extract_event_snapshots(
    ...
    if not isinstance(time_series, np.ndarray) or time_series.ndim != 2:
        raise ValidationError("time_series must be a 2D numpy array", "time_series_ndim", time_series.ndim if hasattr(time_series, 'ndim') else None)
    
    if not isinstance(locations, np.ndarray) or locations.ndim != 1:
        raise ValidationError("locations must be a 1D numpy array", "locations_ndim", locations.ndim if hasattr(locations, 'ndim') else None)
    
    if not isinstance(model_order, int) or model_order <= 0:
        raise ValidationError("model_order must be a positive integer", "model_order", model_order)
    
    if not isinstance(lag_step, int) or lag_step <= 0:
        raise ValidationError("lag_step must be a positive integer", "lag_step", lag_step)
    
    if not isinstance(start_offset, int):
        raise ValidationError("start_offset must be an integer", "start_offset", start_offset)
    
    if not isinstance(extract_length, int) or extract_length <= 0:
        raise ValidationError("extract_length must be a positive integer", "extract_length", extract_length)
    ...
    if min_location < 0 or max_location >= n_time_points:
        raise IndexError(f"Event locations {min_location}-{max_location} out of bounds for time series length {n_time_points}")
...
def perform_desnap_analysis(inputs: DeSnapParams) -> Dict[str, Union[np.ndarray, Dict]]:
    ...
    if not isinstance(inputs, DeSnapParams):
        raise ValidationError("inputs must be a DeSnapParams object", "inputs_type", type(inputs))
    
    try:
        if inputs.d0_max is None and inputs.maxStdRatio is None:
            raise ValidationError("Either d0_max or maxStdRatio must be provided", "d0_max_missing", None)
        
        if inputs.N_d <= 0:
            raise ValidationError("N_d (number of bins) must be positive", "N_d", inputs.N_d)
        
        if inputs.detection_signal.ndim != 1:
            raise ValidationError("detection_signal must be 1D", "detection_signal_ndim", inputs.detection_signal.ndim)
        
        if inputs.original_signal.ndim != 2:
            raise ValidationError("original_signal must be 2D", "original_signal_ndim", inputs.original_signal.ndim)
        ...
        if inputs.detection_signal.shape[0] != inputs.original_signal.shape[1]:
            raise ValidationError(
                ...
                "signal_lengths", (inputs.detection_signal.shape[0], inputs.original_signal.shape[1])
            )

        if inputs.d0_max is None:
            if inputs.maxStdRatio is not None:
                inputs.d0_max = np.mean(inputs.detection_signal) + inputs.maxStdRatio * np.std(inputs.detection_signal)
            else:
                inputs.d0_max = np.max(inputs.detection_signal)

        logger.info(f"DeSnap analysis: d0={inputs.d0}, d0_max={inputs.d0_max}, N_d={inputs.N_d}")
        ...
        try:
            ...
            raise ComputationError(f"First linear regression failed: {e}", "linear_regression_1", None)
        ...
        if not np.any(nan_mask_regression2):
            raise ComputationError("All p_t or q_t values are NaN, cannot compute mu_D", "mu_D_computation", None)
