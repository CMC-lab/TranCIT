...
def compute_causal_strength_nonzero_mean(
    ...
    # Compute covariances
    cov_xp, cov_yp, c_xyp, c_yxp = compute_covariances(lagged_data, n_time_steps, model_order)
    ...
    for t in range(n_time_steps):
        ...
        if time_mode == 'inhomo':
            te_residual_cov[t, 1] = (
                sigy + b.T @ cov_xp_reg @ b - b.T @ c_xyp[t] @ np.linalg.inv(cov_yp_reg) @ c_xyp[t].T @ b
            )
            te_residual_cov[t, 0] = (
                sigx + c.T @ cov_yp_reg @ c - c.T @ c_yxp[t] @ np.linalg.inv(cov_xp_reg) @ c_yxp[t].T @ c
            )
    ...
    if time_mode == 'homo':
        ...
        transfer_entropy[0, 1] = 0.5 * np.log((sigy + b.T @ cov_xp @ b - b.T @ cov_xy_p @ np.linalg.inv(cov_yp) @ cov_xy_p.T @ b) / sigy)
        transfer_entropy[0, 0] = 0.5 * np.log((sigx + c.T @ cov_yp @ c - c.T @ cov_yx_p @ np.linalg.inv(cov_xp) @ cov_yx_p.T @ c) / sigx)
    ...
    return causal_strength, transfer_entropy, granger_causality, coefficients, te_residual_cov
...
def time_varying_causality(
    ...
    for t in range(nobs):
        ...
        causality_measures['TE'][t, 1] = 0.5 * np.log(
            (sigy + b.T @ cov_xp_reg @ b - b.T @ c_xyp @ np.linalg.inv(cov_yp_reg) @ c_xyp.T @ b) / sigy
        )
        causality_measures['TE'][t, 0] = 0.5 * np.log(
            (sigx + c.T @ cov_yp_reg @ c - c.T @ c_yxp @ np.linalg.inv(cov_xp_reg) @ c_yxp.T @ c) / sigx
        )

        mean_x_ref = np.mean(stats['mean'][3::2, ref_time], axis=1)
        mean_y_ref = np.mean(stats['mean'][2::2, ref_time], axis=1)
        cov_xp_ref = (
            cov_xp + mean_xp[:, np.newaxis] @ mean_xp[np.newaxis, :] -
        ...
        cov_yp_ref = (
            cov_yp + mean_yp[:, np.newaxis] @ mean_yp[np.newaxis, :] -
        ...
        if not causal_params['diag_flag']:
            ...
            if causal_params['old_version']:
                cov_xp_lag = (
                    np.dot(
                        lagged_vars - np.mean(stats['mean'][2:, ref_time], axis=1)[:, np.newaxis],
                        (lagged_vars - np.mean(stats['mean'][2:, ref_time], axis=1)[:, np.newaxis]).T
                    ) / ntrials
                )
                causality_measures['rDCS'][t, 1] = (
                    0.5 * np.log((sigy + b.T @ ref_cov_xp @ b) / sigy) - 0.5 +
                    0.5 * (sigy + b.T @ cov_xp_lag[1::2, 1::2] @ b) / (sigy + b.T @ ref_cov_xp @ b)
                )
                causality_measures['rDCS'][t, 0] = (
                    0.5 * np.log((sigx + c.T @ ref_cov_yp @ c) / sigx) - 0.5 +
            ...
            causality_measures['DCS'][t, 1] = 0.5 * np.log((sigy + b.T @ np.diag(np.diag(cov_xp)) @ b) / sigy)
            causality_measures['DCS'][t, 0] = 0.5 * np.log((sigx + c.T @ np.diag(np.diag(cov_yp)) @ c) / sigx)
            ...
            if causal_params['old_version']:
                ...
                causality_measures['rDCS'][t, 1] = (
                    0.5 * np.log((sigy + b.T @ np.diag(np.diag(ref_cov_xp)) @ b) / sigy) - 0.5 +
                    0.5 * (sigy + b.T @ np.diag(np.diag(cov_xp_lag[1::2, 1::2])) @ b) /
                    (sigy + b.T @ np.diag(np.diag(ref_cov_xp)) @ b)
                )
                causality_measures['rDCS'][t, 0] = (
                    0.5 * np.log((sigx + c.T @ np.diag(np.diag(ref_cov_yp)) @ c) / sigx) - 0.5 +
