...
def BIC_compare(Yt_events, morder, BICParser):
    ...
    #     SnapAnalyOutput = snapshot_detect_analysis_pipeline(
    #         BICParser['OriSignal'], BICParser['DetSignal'], Params)
...
def multi_trial_BIC(Yt_events_momax, BICParser):
    ...
    mobic_index = np.nanargmin(BICoutputs['bic'], axis=0)

    BICoutputs['mobic'] = mobic_index + 1
    # BICoutputs['mobic'] = BICoutputs['mobic'] + 1
