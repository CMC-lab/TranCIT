...
config = PipelineConfig(
    ...
    monte_carlo=MonteCParams(n_btsp=100),  # Number of bootstrap samples
    desnap=DeSnapParams(
...
for i in ca3_channels:
    for j in ca1_channels:
        ...
        temp_idx = np.where(
            yf[1, :]
            >= np.mean(yf[1, :]) + config.detection.thres_ratio * np.std(yf[1, :])
        )[0]
        temp_idx = _remove_boundary_locations(
            yf[1, :], temp_idx, config.detection.l_start, config.detection.l_extract
        )
