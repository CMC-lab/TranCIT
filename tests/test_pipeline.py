...
class TestPipelineOrchestrator:
    ...
    def test_initialization(self, minimal_config):
        ...
        expected_stages = [
            'input_validation', 'event_detection', 'border_removal',
            'bic_selection', 'snapshot_extraction', 'artifact_removal', 
            'statistics_computation', 'causality_analysis', 
            'bootstrap_analysis', 'desnap_analysis', 'output_preparation'
        ]
    ...
    def test_run_method_with_mocked_detection(self, mock_detection, sample_signals, minimal_config):
...
class TestPipelineStages:
    ...
    def test_statistics_computation_stage(self, full_config):
        ...
        with patch('dcs.utils.core.compute_event_statistics') as mock_stats:
            mock_stats.return_value = {
                'OLS': {'At': np.random.randn(50, 2, 5), 'Sigma_Et': np.random.randn(50, 2, 2)},
    ...
    def test_causality_analysis_stage(self, full_config):
        ...
        # Create mock inputs
        event_data = np.random.randn(10, 50, 20)  # (nvar * (morder + 1), n_obs, n_trials)
        event_stats = {
            'OLS': {'At': np.random.randn(50, 2, 5), 'Sigma_Et': np.random.randn(50, 2, 2)},
            'Sigma': np.random.randn(50, 6, 6),
            'mean': np.random.randn(6, 50)
...
class TestPipelineErrorHandling:
    ...
    def test_computation_error_handling(self, minimal_config, sample_signals):
        ...
        with patch.object(orchestrator.stages['input_validation'], 'execute', side_effect=Exception("Test error")):
