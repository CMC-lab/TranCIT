...
def sample_data() -> Tuple[np.ndarray, np.ndarray]:
    ...
    data, _, _ = generate_signals(T=300, Ntrial=15, h=0.1, 
                                  gamma1=0.5, gamma2=0.5, 
                                  Omega1=1.0, Omega2=1.2)
    
    original_signal = np.mean(data, axis=2)
    detection_signal = original_signal * 1.3 + np.random.randn(*original_signal.shape) * 0.05
...
class TestFullPipelineIntegration:
    ...
    def test_basic_pipeline_workflow(self, sample_data: Tuple[np.ndarray, np.ndarray], sample_config: PipelineConfig):
...
class TestErrorHandlingIntegration:
    ...
    def test_invalid_data_handling(self, sample_config: PipelineConfig):
        ...
        with pytest.raises(Exception):
            orchestrator.run(
                np.random.randn(2, 100),
                np.random.randn(2, 80)  # Different length
            )
