...
def sample_stats():
    ...
    return {
        ...
        'Sigma': np.random.randn(n_obs, nvar * (model_order + 1), nvar * (model_order + 1)),
...
class TestDCSCalculator:
    ...
    def test_initialization(self):
        ...
        assert calculator.config["use_diagonal_covariance"] == False
        
        calculator_diag = DCSCalculator(model_order=2, use_diagonal_covariance=True)
        assert calculator_diag.config["use_diagonal_covariance"] == True
    ...
    def test_invalid_initialization(self):
        ...
        with pytest.raises(ValidationError, match="time_mode must be 'inhomo' or 'homo'"):
